...
<p>
The calculator is quite simple; the user uploads the gene sets they want to test for enrichment. The app then runs what is essentially a one-tailed Fishers Exact Test with the user\'s uploaded genes for every gene set (currently 171 sets) in the database (against a specified "background universe" of genes). If the user\'s uploaded genes have a significant overlap with any particular gene set, the gene set is said to be highly represented, or "enriched." Depending on the particular gene set that is enriched, this could have important implications for the user\'s gene set.
</p>
